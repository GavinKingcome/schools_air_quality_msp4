...
class Sensor(models.Model):
    ...
    @property
    def is_reference_grade(self):
        return self.network == 'LAQN'
    
    @property
    def is_urban_background(self):
...
class Reading(models.Model):
    """Hourly pollution reading from a sensor. All values in µg/m³."""
    ...
    nox = models.DecimalField(max_digits=6, decimal_places=2, null=True, blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
...
class SensorAnnualStats(models.Model):
    """Annual statistics for calculating adjustment factors."""
    
    sensor = models.ForeignKey(Sensor, on_delete=models.CASCADE, related_name='annual_stats')
    year = models.IntegerField()
    no2_mean = models.DecimalField(max_digits=6, decimal_places=2, null=True, blank=True)
    pm25_mean = models.DecimalField(max_digits=6, decimal_places=2, null=True, blank=True)
    pm10_mean = models.DecimalField(max_digits=6, decimal_places=2, null=True, blank=True)
    o3_mean = models.DecimalField(max_digits=6, decimal_places=2, null=True, blank=True)
    capture_rate = models.DecimalField(max_digits=5, decimal_places=2, null=True, blank=True)
